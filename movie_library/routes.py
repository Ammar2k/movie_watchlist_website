...
def login():
    ...
    if form.validate_on_submit():
        ...
        flash("Login credentials were not correct", category="danger")
...
@pages.get("/movie/<string:_id>rate")  # get id argument from endpoint
@login_required
def rate_movie(_id):
    rating = int(request.args.get("rating"))  # get argument from query string
    current_app.db.movies.update_one(
        {"_id": _id},
        {"$set": {"rating": rating}},
    )
...
# if a value passed to an endpoint is part of an endpoint variable, it will
# populate that variable. Otherwise, it is passed as a query string parameter
